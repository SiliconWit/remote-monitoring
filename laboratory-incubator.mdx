...
$$\Delta T = 37.0 - 35.7 = 1.3\ \text{°C below setpoint}$$
...
![Panel (a): a week of five runs with no temperature record, every one of them shaded as suspect because nothing says when the incubator drifted. Panel (b): the same five runs against a timestamped excursion on Thursday 16 July, where only the single overlapping run is discarded and the other four remain defensible](./images/excursion-and-the-run.svg)
